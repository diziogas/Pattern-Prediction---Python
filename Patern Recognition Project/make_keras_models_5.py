...
with open('timelines/timeline'+str(SAMPLE_LENGTH+5)+'_years_refs.txt','r') as handle:
	i = 0
	for line in handle:
...
for i,ref in enumerate(timeline_refs):
	...
	# 	if r == 0:
	# 		count0 += 1
	if len(ref) == SAMPLE_LENGTH+5:#cut reference list with length > 20
...
TRAIN_SIZE = SAMPLE_LENGTH
GOAL_SIZE = SAMPLE_LENGTH+4
test_size=SAMPLE_LENGTH - TRAIN_SIZE
train,test=dataset[:,0:TRAIN_SIZE],dataset[:,GOAL_SIZE]
...
with open("models5/model"+str(SAMPLE_LENGTH)+".json", "w") as json_file:
    json_file.write(model_json)
# serialize weights to HDF5
model.save_weights("models5/modelweights"+str(SAMPLE_LENGTH)+".h5")
